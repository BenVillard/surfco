...
% 
  ...
  %%Inicializaciones
  if strcmpi(get(0,'DefaultFigureWindowStyle'), 'docked'), set(0,'DefaultFigureWindowStyle','normal'); end
  % GO   -> global options
...
%   J( GO.FIXED_COORDINATES ) = 0;
  ...
  if GO.PLOT
    MultplMntrs = get(0,'MonitorPositions'); 
    POS = 10;
    if size(MultplMntrs,1) > 1, POS = round(MultplMntrs(2,1) - MultplMntrs(2,1)*0.15); end
